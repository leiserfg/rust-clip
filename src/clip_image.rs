use clip_sys::*;
use image::ColorType;

pub struct ClipImage {
  ptr: CClipImage,
}

impl<'a> ClipImage {
  pub fn from_ptr(ptr: CClipImage) -> Self {
    Self { ptr }
  }

  pub fn get_spec(&self) -> CClipImageSpec {
    unsafe { clip_get_image_spec(self.ptr) }
  }

  pub fn get_data(&self) -> &'a [u8] {
    let spec = self.get_spec();
    let len: usize = spec.bytes_per_row as usize * spec.height as usize;
    unsafe { std::slice::from_raw_parts(clip_get_image_data(self.ptr) as *const u8, len) }
  }
}

impl Drop for ClipImage {
  fn drop(&mut self) {
    unsafe {
      clip_delete_image(self.ptr);
    }
  }
}

#[test]
fn test_png() {
  use super::Clip;

  let mut output_writer = std::fs::File::create("test.png").unwrap();
  let encoder = image::png::PNGEncoder::new(&mut output_writer);

  let clip_image = Clip::get_image().unwrap();
  let clip_spec = clip_image.get_spec();
  let clip_data = clip_image.get_data();

  encoder
    .encode(
      clip_data,
      clip_spec.width as u32,
      clip_spec.height as u32,
      ColorType::RGBA(8),
    )
    .unwrap();
}

#[test]
fn test_jpeg() {
  use super::Clip;

  let mut output_writer = std::fs::File::create("test.jpeg").unwrap();
  let mut encoder = image::jpeg::JPEGEncoder::new(&mut output_writer);

  let clip_image = Clip::get_image().unwrap();
  let clip_spec = clip_image.get_spec();
  let clip_data = clip_image.get_data();

  encoder
    .encode(
      clip_data,
      clip_spec.width as u32,
      clip_spec.height as u32,
      ColorType::RGBA(8),
    )
    .unwrap();
}
